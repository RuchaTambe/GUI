...
add_to_events():
	...
	An object collects the data sent by camera and it is  written to a file camera1.py.
Long polling is implemented with the use of a global variable- user_requested, initially set to False. While there is no log request incoming from the client, the server is in a wait state,
Implemented by a while loop in function. 
